...
bool ImGui::MainTab(const char* label, ImFont* icon_font, const char* icon, bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    RenderTextClipped(bb.Min + ImVec2(size_arg.x * 0.33f, (4 * dpi_scale) - 4), bb.Max, label, NULL, &label_size, ImVec2(0.f, 0.41f), &bb);
    PopStyleColor();
...
bool ImGui::KeybindButton(const char* label, const char* unique_id, const ImVec2& size_arg, bool clicked, ImGuiButtonFlags flags)
{
    ...
    it_hover->second = std::clamp(it_hover->second + (3.f * ImGui::GetIO().DeltaTime * ((hovered || clicked) ? 1.f : -1.f)), 0.0f, 1.f);
    ...
    RenderFrame(bb.Min, bb.Max, col, true, 2.f * dpi_scale);
    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(ImVec4(126 / 255.f, 131 / 255.f, 219 / 255.f, it_hover->second)), 2.f * dpi_scale);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    {
        const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 8.0f));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(frame_bb.Min, grab_bb.Max, IsItemHovered()? GetColorU32(ImGuiCol_SliderGrabActive) : GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const bool is_multiline = 0;
    const bool is_readonly = 1;
    ...
    draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos, col, buf);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (1)
        {
            ...
            temp_buf = temp_buf.substr(0, cart_pos);
            cursor_offset.x = CalcTextSize(temp_buf.c_str()).x;
            state->CursorAnim += io.DeltaTime;
            bool cursor_is_visible = (!g.IO.ConfigInputTextCursorBlink) || (state->CursorAnim <= 0.0f) || ImFmod(state->CursorAnim, 1.20f) <= 0.80f;
            ImVec2 cursor_screen_pos = ImFloor(draw_pos + cursor_offset);
