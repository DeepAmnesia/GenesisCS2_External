...
{
	this->Address.ClientDLL = reinterpret_cast<DWORD64>(ProcessMgr.GetProcessModuleHandle(_S("client.dll")));
...
{
	Vector3 Angle{ Pitch,Yaw };

	if (!ProcessMgr.WriteMemory<Vector3>(this->Address.ViewAngle, Angle))
		return false;
